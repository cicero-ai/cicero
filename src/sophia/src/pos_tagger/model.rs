...
use super::{
    POSContext, POSFeature, POSPrediction, POSPredictionMethod, POSPrefix, POSSuffix, POSTag,
    SIBLING_TAGS_AFTER, SIBLING_TAGS_BEFORE, TokenKey,
};
    ...
    Token: TokenKey<S>,
{
    ...
    fn predict(&self, position: usize, tokens: &[Token]) -> Option<POSPrediction> {
        ...
        Some(POSPrediction::new(
            POSPredictionMethod::standard,
...
{
    /// Check conjunctions
    fn check_conjunctions(
        &self,
        position: usize,
        context: &POSContext<S>,
        context_vec: &HashSet<POSFeature<S>>,
        tokens: &[Token],
    ) -> Option<POSPrediction> {
        ...
        for feature in context_vec.iter() {
            ...
            for conjunction in conjunction_set.iter() {
                ...
                if let Some(tag) = self.check_exceptions(conjunction, context_vec) {
                    ...
                    return Some(POSPrediction::new(
                        POSPredictionMethod::exception,
                        &tokens[position].word,
                        tokens[position].pos,
                        tag.unwrap(),
                        1.0,
                ...
                if !conjunction.siblings.iter().all(|sib| {
                    let offset =
                        ((feature.offset + sib.offset) + (SIBLING_TAGS_BEFORE as i8)) as usize;
                ...
                if let Some(tag) = conjunction.deterministic_tag {
                    return Some(POSPrediction::new(
                        POSPredictionMethod::deterministic_rule,
                        &tokens[position].word,
                        tokens[position].pos,
                        tag,
                        1.0,
        ...
        Some(POSPrediction::new(
            POSPredictionMethod::conjunction,
            &tokens[position].word,
            tokens[position].pos,
            *scores_vec[0].0,
            *scores_vec[0].1,
    ...
    fn check_exceptions(
        &self,
        conjunction: &POSConjunction<S>,
        context_vec: &HashSet<POSFeature<S>>,
    ) -> Option<Option<POSTag>> {
        for (exception, opt_tag) in conjunction.exceptions.iter() {
            if !context_vec.contains(exception) {
                continue;
            }
    ...
    pub fn predict_cohort(&self, position: usize, tokens: &[Token]) -> Option<POSPrediction> {
        ...
        if let Ok(prefix) = POSPrefix::try_from(&tokens[position]) {
            let (prefix_tag, prefix_bonus) = match prefix {
                ...
                | POSPrefix::mono
                | POSPrefix::pseudo
                | POSPrefix::quasi => (POSTag::JJ, 0.075),
                POSPrefix::en | POSPrefix::em | POSPrefix::mis => (POSTag::VB, 0.075),
                POSPrefix::sub | POSPrefix::inter | POSPrefix::intra | POSPrefix::trans => {
                    (POSTag::NN, 0.075)
                }
                POSPrefix::un
                | POSPrefix::pre
                ...
                | POSPrefix::mega
                | POSPrefix::extra
                | POSPrefix::poly => (POSTag::JJ, 0.05),
                POSPrefix::re | POSPrefix::dis | POSPrefix::de => (POSTag::VB, 0.05),
                POSPrefix::co | POSPrefix::com | POSPrefix::post | POSPrefix::fore => {
                    (POSTag::NN, 0.05)
                }
                _ => (POSTag::FW, 0.0),
            };
    ...
    Token: TokenKey<S>,
{
    ...
    pub fn add_feature(&mut self, feature: &POSFeature<S>, weight: &POSWeight) {
        ...
        // Primary feature
        } else if self.primary[index].is_none()
            || feature.get_score() > self.primary[index].as_ref().unwrap().feature.get_score()
        {
            let tags: HashMap<POSTag, f32> =
                weight.tags.iter().map(|(tag, score)| (*tag, (weight.weight * *score))).collect();
