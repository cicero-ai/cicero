...
use crate::vocab::{VocabMWE, Capitalization};
use super::{POSTag, POSPrediction, POSPredictionMethod, TokenKey};
    ...
    where S: Default + Clone + Eq + PartialEq + Hash + Serialize + for<'a> Deserialize<'a>, Token: TokenKey<S>
{
    ...
    fn viterbi_decode(&self, start_pos: usize, end_pos: usize, tokens: &mut [Token]) {
        ...
        for (offset, token) in tokens[start_pos..end_pos].iter().enumerate() {
            ...
            if offset == 0 {
                ...
                let tag_indices = if token.potential_pos.len() > 1 {
                    token.potential_pos.iter().filter(|&tag| *tag != POSTag::FW).map(|tag| tag.to_u8() as usize).collect::<Vec<usize>>()
                ...
                let current_tag_idx = if tag_indices.len() == 1 { tag_indices[0] } else { 0 };
                let mut probs = Probability::new(current_tag_idx);

                for tag_idx in tag_indices {
                    probs.viterbi[tag_idx] = self.initial_probs[tag_idx] + self.get_emission_prob(tag_idx, token);
                }
        ...
        for (offset, tag_idx) in path.iter().enumerate() {
            ...
            if tokens[position].potential_pos.len() < 2 && tokens[position].index > 0 {
                ...
                tokens[position].pos_prediction = POSPrediction::new(POSPredictionMethod::hmm, &tokens[position].word, tokens[position].pos, tag, confidence, &HashMap::new(), &[]);
                tokens[position].pos = tag;
            }

            // Check for named entity
            let tag = tokens[position].pos;
            if tag == POSTag::NN && VocabMWE::classify_capitalization(&tokens[position].word) != Capitalization::lower && !is_initial {
    ...
    fn calculate_viterbi(&self, position: usize, results: &[Probability], tokens: &[Token]) -> Probability {

        // Instantiate probability
        let token = &tokens[position];
        let deterministic_tag_idx = if token.potential_pos.len() > 1 || token.pos == POSTag::FW { 0 } else { token.pos.to_u8() as usize };
        ...
        let prev_tag_indices = if tokens[position-1].potential_pos.len() > 1 {
            tokens[position-1].potential_pos.iter().filter(|&tag| *tag != POSTag::FW).map(|tag| tag.to_u8() as usize).collect::<Vec<usize>>()
        ...
        for tag_idx in tag_indices.iter() {
            ...
            for prev_tag_idx in prev_tag_indices.iter() {
                let score = prev_probs.viterbi[*prev_tag_idx] + self.transmition_probs[*prev_tag_idx][*tag_idx] + emission_prob;
    ...
    fn get_emission_prob(&self, tag_idx: usize, token: &Token) -> f32 {
        ...
        match self.emission_probs[tag_idx].get(&token.get_key()) {
            ...
            None => {
                ...
                (self.smoothing as f32 / (tag_vocab_size + self.vocab_size * self.smoothing as f32)).ln()
            }
        }
    }

    /// Returns a value between 0.0 and 1.0, where 1.0 means completely certain
    fn get_confidence_score(&self, position: usize, offset: usize, results: &[Probability], tokens: &[Token]) -> f32 {
        let token = &tokens[position];
        let prob_result = &results[offset];

        let tag_indices: Vec<usize> = token.potential_pos.iter()
            .filter(|&tag| *tag != POSTag::FW)
            .map(|tag| tag.to_u8() as usize)
            .collect();

        // Get scores for all possible tags at this position
        let mut scores: Vec<f32> = tag_indices.iter()
        ...
        let prob_scores: Vec<f32> = scores.iter()
            .map(|&score| (score - max_score).exp())
            .collect();

        let total_prob: f32 = prob_scores.iter().sum();
        let normalized_probs: Vec<f32> = prob_scores.iter()
            .map(|&prob| prob / total_prob)
            .collect();
